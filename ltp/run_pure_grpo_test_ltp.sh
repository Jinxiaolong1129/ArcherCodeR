#!/usr/bin/env bash
# ============================================================
# Pure GRPO smoke test launcher for the LTP / PAI environment.
# Runs INSIDE the GPU container (single node, 8 GPU).
# Adapted from:
#   Archer_eval/tools/slurm_example/run_Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-temp0.8.sh
# Differences vs the slurm version:
#   - all paths are absolute on cephfs (no external network on GPU node)
#   - MODEL_PATH points at a pre-staged model on cephfs (no HF download)
#   - logger = console only (no wandb / .env needed for the smoke test)
#   - shorter run knobs so the test finishes fast
# ============================================================
set -xeuo pipefail
...
MODEL_PATH=${MODEL_PATH:-$CEPHFS/models/DeepSeek-R1-Distill-Qwen-1.5B}

cd "$REPO_DIR"

project_name='ArcherCodeR'
exp_name='Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-temp0.8-ltptest-optA'
...
# Data (pre-staged on cephfs by tools/download_datasets.py)
...
# Performance
# --- opt A (free / quality-neutral speedups) ---
#   gen_tp=1   : 1.5B fits on one h200 -> 8 vLLM replicas (was TP=2 -> 4 replicas)
#   dynamic_bsz: pack sequences by token budget instead of micro_batch=1
# token budget kept at 10240 (= prompt+resp) to avoid OOM (mem already ~85%).
sp_size=1
gen_tp=1
...
echo "🚀 PURE GRPO SMOKE TEST (LTP) | model=${MODEL_PATH} | gpus=8 | nnodes=${nnodes}"
