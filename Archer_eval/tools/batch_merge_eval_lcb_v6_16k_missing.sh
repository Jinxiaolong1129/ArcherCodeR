...
#
# 缺失列表:
# - Archer-TokenEntropy-...-temp0.8: step 50
# - Archer-TrajectoryEntropy-...-n12: steps 10, 50, 80, 100
# - Archer-TrajectoryEntropy-...-temp1.2: steps 10, 50, 80, 100
#
...
lcb_max_response_length=$((1024 * 16))    # 16K response
...
mkdir -p ${LOG_DIR}
MAIN_LOG=${LOG_DIR}/batch_merge_eval_lcb_v6_16k_missing_$(date +%Y%m%d_%H%M%S).log

# ============ 缺失任务列表 ============
# 格式: "实验名:步数"
MISSING_TASKS=(
    "Archer-TokenEntropy-Qwen2.5-1.5B-2k-8k-batch64-no-kl-temp0.8:50"
...
main() {
    ...
    log "16K 输出长度版本 - 补充缺失的评估"
    log "============================================================"
    log "缺失任务数量: ${#MISSING_TASKS[@]}"
    log "GPU: ${CUDA_VISIBLE_DEVICES} (${n_gpus} GPUs)"
    log "LCB参数: n_samples=${lcb_n_samples}, prompt=${lcb_max_prompt_length}, response=${lcb_max_response_length} (16K)"
    ...
    for task in "${MISSING_TASKS[@]}"; do
        ...
        log ""
        log "########################################################"
        log "任务 ${lcb_total}/${#MISSING_TASKS[@]}: ${exp_name} step ${step}"
        log "########################################################"
