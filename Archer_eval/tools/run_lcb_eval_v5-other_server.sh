...
#
# LCB v5 自动化评估脚本
# 扫描 output/ArcherCodeR/ 下的模型，自动评估 livecodebench_v5
# 支持 output/ 和 output_16k/ 两种目录
#
# 用法:
#   bash run_lcb_eval_v5.sh                    # 单次扫描并评估
#   DRY_RUN=1 bash run_lcb_eval_v5.sh          # 干运行，只显示待评估任务
#   MODE=watch bash run_lcb_eval_v5.sh         # 持续监控模式
#   TARGET_DIRS="exp1 exp2" bash run_lcb_eval_v5.sh  # 指定实验目录
#   NUM_WORKERS=64 bash run_lcb_eval_v5.sh     # 设置 CPU workers 数量
#
...
MODEL_ROOT="${BASE_DIR}/output/ArcherCodeR"
LOG_FILE="${BASE_DIR}/tools/logs/lcb_v5_evaluated_tasks.log"
...
run_evaluation() {
    ...
    # 提取实验名和步数
    # 路径格式: .../EXP_NAME/global_step_XXX/actor/hf_model/output[_16k]/livecodebench_v5.parquet
    ...
    local output_type=$(basename "$(dirname "$eval_file")")  # output or output_16k
    
    # 如果是 output_16k，在实验名后加后缀
    if [[ "$output_type" == "output_16k" ]]; then
        exp_name="${exp_name}_16k"
    fi
...
process_model() {
    ...
    # 检查两种可能的输出目录
    for output_dir in "output" "output_16k"; do
        local eval_file="${model_path}/actor/hf_model/${output_dir}/livecodebench_v5.parquet"
...
scan_and_process() {
    ...
    log "待评估任务: ${PENDING_COUNT} 个"
    
    if [[ "$DRY_RUN" == "1" ]] && [[ "$PENDING_COUNT" -gt 0 ]]; then
        log ""
        log "💡 要执行评估，请运行:"
        log "   bash tools/run_lcb_eval_v5.sh"
    fi
    log "=============================================="
}
