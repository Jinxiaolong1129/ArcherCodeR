#!/bin/bash
#
# 批量模型合并与 LiveCodeBench 评估脚本 (SLURM版本 - 4 GPU)
# 16K 输出长度版本 - Part 2
#
...
lcb_max_response_length=$((1024 * 16))    # 16K response (关键修改)
...
mkdir -p ${LOG_DIR}
MAIN_LOG=${LOG_DIR}/batch_merge_eval_lcb_16k_part2_$(date +%Y%m%d_%H%M%S).log
...
main() {
    log "============================================================"
    log "批量模型合并与 LiveCodeBench 评估脚本 (SLURM - 4 GPU)"
    log "16K 输出长度版本 - Part 2"
    log "============================================================"
