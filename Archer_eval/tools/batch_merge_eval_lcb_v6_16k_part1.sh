#!/bin/bash
#
# 批量模型合并与 LiveCodeBench v6 评估脚本 (SLURM版本 - 3 GPU)
# 16K 输出长度版本 - Part 1
#
...
mkdir -p ${LOG_DIR}
MAIN_LOG=${LOG_DIR}/batch_merge_eval_lcb_v6_16k_part1_$(date +%Y%m%d_%H%M%S).log
...
main() {
    log "============================================================"
    log "批量模型合并与 LiveCodeBench v6 评估脚本 (SLURM - 3 GPU)"
