#!/usr/bin/env python
# /Archer_eval/verl/trainer/compute_certainty.py
...
def parallel_compute_score(evaluation_func, response_str, ground_truth, data_sources, max_workers=25):
...
def main_task(config):
    ...
    # Get data_sources and ground_truth for correctness evaluation
    ...
    # Compute correctness scores - skip LCB (slow/OOM), only evaluate AIME
    print("Computing correctness scores (AIME only, skipping LCB)...")
    all_scores = []
    
    # Separate AIME and LCB indices
    aime_indices = []
    lcb_indices = []
    for i, ds in enumerate(all_data_sources):
        if 'livecodebench' in ds.lower():
            lcb_indices.append(i)
        else:
            aime_indices.append(i)
    
    print(f"  AIME samples: {len(aime_indices)}, LCB samples (skipped): {len(lcb_indices)}")
    
    # Initialize all scores to -1 (not evaluated)
    all_scores = [-1.0] * len(all_responses)
    
    # Only evaluate AIME
    if aime_indices:
    try:
        from rewards.general_reward import general_reward_fn
            aime_responses = [all_responses[i] for i in aime_indices]
            aime_ground_truths = [all_ground_truths[i] for i in aime_indices]
            aime_data_sources = [all_data_sources[i] for i in aime_indices]
            
            aime_scores = parallel_compute_score(
            general_reward_fn,
                aime_responses,
                aime_ground_truths,
                aime_data_sources,
            ...
            # Convert boolean scores to float and assign back
            for idx, score in zip(aime_indices, aime_scores):
                all_scores[idx] = 1.0 if score else 0.0
                
            print(f"  AIME accuracy: {sum(1 for s in aime_scores if s) / len(aime_scores) * 100:.2f}%")
    except Exception as e:
            print(f"  Error computing AIME scores: {e}. Setting all to -1.")
    
    print(f"  LCB samples marked as -1 (not evaluated)")
    ...
    for i, (row_idx, resp_idx) in enumerate(all_indices):
        results.append({
            ...
            'is_correct': all_scores[i],  # -1 = not evaluated
    ...
    # Separate evaluated (AIME) and non-evaluated (LCB) samples
    evaluated_df = results_df[results_df['is_correct'] >= 0]
    non_evaluated_df = results_df[results_df['is_correct'] < 0]
    
    summary_data = [
        ['Total samples', len(results_df)],
        ['Evaluated (AIME)', len(evaluated_df)],
        ['Not evaluated (LCB)', len(non_evaluated_df)],
    ]
    
    aime_accuracy = None
    if len(evaluated_df) > 0:
        correct_df = evaluated_df[evaluated_df['is_correct'] == 1.0]
        incorrect_df = evaluated_df[evaluated_df['is_correct'] == 0.0]
        aime_accuracy = len(correct_df) / len(evaluated_df)
        summary_data.extend([
            ['AIME Correct', len(correct_df)],
            ['AIME Incorrect', len(incorrect_df)],
            ['AIME Accuracy', f"{aime_accuracy * 100:.2f}%"],
        ])
    
    summary_data.extend([
        ['', ''],
        ['Metric', 'Mean | Std | Min | Max'],
        ['self_certainty', f"{results_df['self_certainty'].mean():.4f} | {results_df['self_certainty'].std():.4f} | {results_df['self_certainty'].min():.4f} | {results_df['self_certainty'].max():.4f}"],
        ['entropy', f"{results_df['entropy'].mean():.4f} | {results_df['entropy'].std():.4f} | {results_df['entropy'].min():.4f} | {results_df['entropy'].max():.4f}"],
        ['prob_disparity', f"{results_df['prob_disparity'].mean():.4f} | {results_df['prob_disparity'].std():.4f} | {results_df['prob_disparity'].min():.4f} | {results_df['prob_disparity'].max():.4f}"],
        ['trajectory_entropy', f"{results_df['trajectory_entropy'].mean():.4f} | {results_df['trajectory_entropy'].std():.4f} | {results_df['trajectory_entropy'].min():.4f} | {results_df['trajectory_entropy'].max():.4f}"],
    ])
    ...
    summary = {
        ...
        'evaluated_samples': len(evaluated_df),
        'non_evaluated_samples': len(non_evaluated_df),
        'aime_accuracy': aime_accuracy,
        'metrics': {
            'self_certainty': {
                'mean': float(results_df['self_certainty'].mean()),
                'std': float(results_df['self_certainty'].std()),
                'min': float(results_df['self_certainty'].min()),
                'max': float(results_df['self_certainty'].max()),
            },
            'entropy': {
                'mean': float(results_df['entropy'].mean()),
                'std': float(results_df['entropy'].std()),
                'min': float(results_df['entropy'].min()),
                'max': float(results_df['entropy'].max()),
            },
            'prob_disparity': {
                'mean': float(results_df['prob_disparity'].mean()),
                'std': float(results_df['prob_disparity'].std()),
                'min': float(results_df['prob_disparity'].min()),
                'max': float(results_df['prob_disparity'].max()),
            },
            'trajectory_entropy': {
                'mean': float(results_df['trajectory_entropy'].mean()),
                'std': float(results_df['trajectory_entropy'].std()),
                'min': float(results_df['trajectory_entropy'].min()),
                'max': float(results_df['trajectory_entropy'].max()),
            },
        }
    }
