...
exp_name='Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-from-tokenentropy-step105-batch64'
...
# TokenEntropy checkpoint step 105 - full checkpoint for resume
TOKENENTROPY_CKPT_PATH=./output/ArcherCodeR/Archer-TokenEntropy-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple/global_step_105
TOKENENTROPY_HF_MODEL_PATH=${TOKENENTROPY_CKPT_PATH}/actor/hf_model
...
echo "🚀 PURE GRPO CONFIGURATION (FROM TOKEN ENTROPY CHECKPOINT STEP 105):"
...
echo "🎯 Algorithm: Switching from TokenEntropy to Pure GRPO"
echo "🔄 Resume checkpoint: ${TOKENENTROPY_CKPT_PATH}"
echo "🔄 Model path: ${TOKENENTROPY_HF_MODEL_PATH}"
...
/data/xuandong_zhao/anaconda3/envs/archer/bin/python -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.model.path="${TOKENENTROPY_HF_MODEL_PATH}" \
    ...
    trainer.total_training_steps=155 \
    trainer.default_local_dir="${CKPTS_DIR}" \
    trainer.resume_from_path="${TOKENENTROPY_CKPT_PATH}" \
