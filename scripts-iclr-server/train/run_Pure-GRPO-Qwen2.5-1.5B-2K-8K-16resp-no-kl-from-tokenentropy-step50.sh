...
exp_name='Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-from-tokenentropy-step50-batch64'
...
# Resume from TokenEntropy checkpoint
TOKENENTROPY_CHECKPOINT_PATH=./output/ArcherCodeR/Archer-TokenEntropy-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple/global_step_50
...
echo "🚀 PURE GRPO CONFIGURATION (FROM TOKEN ENTROPY CHECKPOINT STEP 50):"
...
echo "🔄 Resume from: ${TOKENENTROPY_CHECKPOINT_PATH}"
...
/data/xuandong_zhao/anaconda3/envs/archer/bin/python -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.model.path="${TOKENENTROPY_CHECKPOINT_PATH}/actor/hf_model" \
    ...
    trainer.resume_from_path="${TOKENENTROPY_CHECKPOINT_PATH}" \
