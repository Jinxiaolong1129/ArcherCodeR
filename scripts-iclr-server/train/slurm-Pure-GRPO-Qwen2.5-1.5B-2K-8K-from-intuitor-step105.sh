...
#SBATCH --output=./output/ArcherCodeR/Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-from-intuitor-step105/slurm_out_%j.txt
#SBATCH --error=./output/ArcherCodeR/Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-from-intuitor-step105/slurm_error_%j.txt
#SBATCH --job-name=grpo-from-intuitor-step105
...
# 创建输出目录
mkdir -p ./output/ArcherCodeR/Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-from-intuitor-step105

# ============================================
# Step 1: Merge FSDP checkpoint to HF format
# ============================================
INTUITOR_CHECKPOINT_PATH=./output/ArcherCodeR/Archer-Intuitor-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple-v2/global_step_105

# Check if hf_model already exists
if [ -d "${INTUITOR_CHECKPOINT_PATH}/actor/hf_model" ]; then
    ...
    /data/xuandong_zhao/anaconda3/envs/archer/bin/python -m tools.model_merge merge \
        --backend fsdp \
        --local_dir ${INTUITOR_CHECKPOINT_PATH}/actor \
        --target_dir ${INTUITOR_CHECKPOINT_PATH}/actor/hf_model
...
bash scripts-iclr-server/train/run_Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-from-intuitor-step105.sh
