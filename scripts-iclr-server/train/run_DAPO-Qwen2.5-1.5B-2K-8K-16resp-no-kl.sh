...
exp_name='DAPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl'
...
dynamic_sampling_metric=seq_final_reward
max_num_gen_batches=5

echo "🚀 DAPO CONFIGURATION (NO KL LOSS):"
...
echo "🔄 Dynamic Sampling: ${enable_dynamic_sampling}"
echo "📊 Sampling Metric: ${dynamic_sampling_metric}"

mkdir -p "${CKPTS_DIR}"
mkdir -p "${CKPTS_DIR}/eval"

/data/xuandong_zhao/anaconda3/envs/archer/bin/python -m dapo.main_dapo_ori \
    ...
    algorithm.filter_groups.enable=${enable_dynamic_sampling} \
    algorithm.filter_groups.metric=${dynamic_sampling_metric} \
    ...
    +trainer.validation_data_dir=${CKPTS_DIR}/eval $@ 2>&1 | tee ${CKPTS_DIR}/${project_name}_${exp_name}_dapo.log 
