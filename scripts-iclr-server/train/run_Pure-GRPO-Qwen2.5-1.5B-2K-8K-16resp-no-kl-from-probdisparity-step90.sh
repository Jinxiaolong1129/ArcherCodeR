...
exp_name='Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-from-probdisparity-step90'
...
# ProbDisparity checkpoint step 90 - hf_model already exists
PROBDISPARITY_HF_MODEL_PATH=./output/ArcherCodeR/Archer-ProbDisparity-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple/global_step_90/actor/hf_model
...
echo "🚀 PURE GRPO CONFIGURATION (FROM PROB DISPARITY CHECKPOINT STEP 90):"
...
echo "🎯 Algorithm: Switching from ProbDisparity to Pure GRPO"
echo "🔄 Resume from: ${PROBDISPARITY_HF_MODEL_PATH}"
...
/data/xuandong_zhao/anaconda3/envs/archer/bin/python -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.model.path="${PROBDISPARITY_HF_MODEL_PATH}" \
