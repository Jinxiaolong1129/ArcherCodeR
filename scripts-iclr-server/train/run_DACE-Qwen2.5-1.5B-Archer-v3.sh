...
# ================================
# DACE Training Script v3 for ArcherCodeR
# Exploring more aggressive hyperparameters
# α_scale=0.15 (3x paper), β_threshold=0.6 (much stricter)
# ================================
...
exp_name='DACE-v3-Qwen2.5-1.5B-alpha0.15-beta0.6'

# ================================
# DACE Configuration v3
# ================================
adv_estimator=dace

# DACE-specific hyperparameters (AGGRESSIVE exploration)
dace_alpha_scale=0.15          # 3x from paper (0.05) → very strong intrinsic reward
dace_beta_threshold=0.6        # Much stricter (0.4→0.5→0.6) → need 60% success to be easy
...
echo "🚀 DACE v3 CONFIGURATION (Aggressive Exploration):"
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║ DACE v3: Maximum Intrinsic Reward & Strictest Difficulty     ║"
echo "╚═══════════════════════════════════════════════════════════════╝"
...
echo ""
echo "🎲 DACE v3 Hyperparameters (AGGRESSIVE):"
echo "  ├─ α_scale: ${dace_alpha_scale} (paper: 0.05, v1: 0.05, v2: 0.1, v3: 0.15 🚀 3x!)"
echo "  ├─ β_threshold: ${dace_beta_threshold} (paper: 0.4, v1: 0.4, v2: 0.5, v3: 0.6 ⬆️⬆️)"
echo "  └─ Advantage normalization: ${norm_adv_by_std_in_grpo}"
echo ""
echo "💡 Expected Effects of v3 Changes:"
echo "  🚀 α_scale 0.05→0.15 (3x original):"
echo "     • VERY strong intrinsic reward signal (~30-35% contribution vs ~8%)"
echo "     • Maximum exploration on hard tasks"
echo "     • Large penalties for uncertain responses"
echo "     • Risk: May dominate external reward if too strong"
echo ""
echo "  📈 β_threshold 0.4→0.6 (much stricter):"
echo "     • Most tasks classified as 'hard' (need >60% success to be easy)"
echo "     • Majority of samples get negative α (exploration mode)"
echo "     • Very difficult to switch to exploitation mode"
echo "     • Risk: May over-explore even on learnable tasks"
echo ""
echo "💡 How DACE v3 works:"
...
echo "  • For HARD tasks (diff > 0.6): α = -0.15 → AGGRESSIVE explore (3x stronger!)"
echo "  • For EASY tasks (diff < 0.6): α = +0.15 → AGGRESSIVE exploit (3x stronger!)"
echo "  • Total reward: R = R_ext + α × C"
echo ""
echo "⚠️  WARNING: Very aggressive parameters - may need careful monitoring!"
echo "╚═══════════════════════════════════════════════════════════════╝"
...
/home/ec2-user/miniconda3/envs/archer/bin/python -m dapo.main_dapo \
    ...
    +trainer.rejection_sample=True $@ 2>&1 | tee ${CKPTS_DIR}/${project_name}_${exp_name}_dace-v3.log

echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║ ✅ DACE v3 Training Completed                                 ║"
echo "║ 📁 Logs: ${CKPTS_DIR}/${project_name}_${exp_name}_dace-v3.log ║"
