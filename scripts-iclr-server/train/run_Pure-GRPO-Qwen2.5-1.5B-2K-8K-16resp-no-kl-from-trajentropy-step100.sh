...
exp_name='Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-from-trajentropy-step100'
...
# TrajectoryEntropy checkpoint step 100 - hf_model already exists
TRAJENTROPY_HF_MODEL_PATH=./output/ArcherCodeR/Archer-TrajectoryEntropy-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple/global_step_100/actor/hf_model
...
echo "🚀 PURE GRPO CONFIGURATION (FROM TRAJECTORY ENTROPY CHECKPOINT STEP 100):"
...
echo "🔄 Resume from: ${TRAJENTROPY_HF_MODEL_PATH}"
...
/data/xuandong_zhao/anaconda3/envs/archer/bin/python -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.model.path="${TRAJENTROPY_HF_MODEL_PATH}" \
