...
#SBATCH --output=./output/ArcherCodeR/Pure-GRPO-LCB-Qwen2.5-1.5B-2K-8K-16resp-no-kl-v4/slurm_out_%j.txt
#SBATCH --error=./output/ArcherCodeR/Pure-GRPO-LCB-Qwen2.5-1.5B-2K-8K-16resp-no-kl-v4/slurm_error_%j.txt
#SBATCH --job-name=pure-grpo-lcb-no-kl
...
mkdir -p ./output/ArcherCodeR/Pure-GRPO-LCB-Qwen2.5-1.5B-2K-8K-16resp-no-kl-v4

# Clean up Ray cache before training
ray stop --force 2>/dev/null || true
rm -rf /tmp/ray/session_* 2>/dev/null || true

bash scripts-iclr-server/train/run_Pure-GRPO-Qwen2.5-1.5B-LCB-16resp-no-kl.sh
