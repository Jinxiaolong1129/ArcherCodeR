...
exp_name='Pure-GRPO-Qwen2.5-1.5B-2K-8K-16resp-no-kl-from-trajentropy-step105'
...
# Resume from TrajectoryEntropy checkpoint step 105
TRAJENTROPY_CHECKPOINT_PATH=./output/ArcherCodeR/Archer-TrajectoryEntropy-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple/global_step_105
...
echo "🚀 PURE GRPO CONFIGURATION (FROM TRAJECTORY ENTROPY CHECKPOINT STEP 105):"
...
echo "🔄 Resume from: ${TRAJENTROPY_CHECKPOINT_PATH}"
...
/data/xuandong_zhao/anaconda3/envs/archer/bin/python -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.model.path="${TRAJENTROPY_CHECKPOINT_PATH}/actor/hf_model" \
    ...
    trainer.resume_from_path="${TRAJENTROPY_CHECKPOINT_PATH}" \
