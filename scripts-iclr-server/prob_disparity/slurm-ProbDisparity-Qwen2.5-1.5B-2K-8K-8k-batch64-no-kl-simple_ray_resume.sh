...
#SBATCH --output=./output/ArcherCodeR/Archer-ProbDisparity-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple/slurm_out_%j.txt
#SBATCH --error=./output/ArcherCodeR/Archer-ProbDisparity-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple/slurm_error_%j.txt
#SBATCH --job-name=prob-disparity-resume
...
mkdir -p ./output/ArcherCodeR/Archer-ProbDisparity-Qwen2.5-1.5B-2k-8k-batch64-no-kl-simple
...
# Run the training script with explicit Ray CPU configuration (RESUME mode)
bash scripts-iclr-server/prob_disparity/prob_disparity_Qwen-1.5B-2k-8k-8k-batch64-no-kl-simple_ray_resume.sh ray_init.num_cpus=160
