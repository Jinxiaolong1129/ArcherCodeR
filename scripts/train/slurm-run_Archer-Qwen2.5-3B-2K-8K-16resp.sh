...
#SBATCH --output=./output/ArcherCodeR/Archer-Qwen2.5-3B-2K-8K-16resp/slurm_out_%j.txt
...
mkdir -p ./output/ArcherCodeR/Archer-Qwen2.5-3B-2K-8K-16resp

# Stop any existing Ray processes
ray stop --force 2>/dev/null || true

# Run the training script with explicit Ray CPU configuration
bash scripts/train/run_Archer-Qwen2.5-3B-2K-8K-16resp.sh ray_init.num_cpus=160 