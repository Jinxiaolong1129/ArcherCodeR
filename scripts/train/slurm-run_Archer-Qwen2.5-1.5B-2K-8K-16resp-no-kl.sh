...
#SBATCH --output=./output/ArcherCodeR/Archer-Qwen2.5-1.5B-2K-8K-16resp-no-kl/slurm_out_%j.txt
#SBATCH --error=./output/ArcherCodeR/Archer-Qwen2.5-1.5B-2K-8K-16resp-no-kl/slurm_error_%j.txt
#SBATCH --job-name=archer-qwen2.5-1.5b-2k-8k-16resp-no-kl
...
mkdir -p ./output/ArcherCodeR/Archer-Qwen2.5-1.5B-2K-8K-16resp-no-kl

# Stop any existing Ray processes
ray stop --force 2>/dev/null || true

# Run the training script with explicit Ray CPU configuration
bash scripts/train/run_Archer-Qwen2.5-1.5B-2K-8K-16resp-no-kl.sh ray_init.num_cpus=160 
