...
exp_name='Archer-Intuitor-Qwen2.5-3B-2k-4k-batch64'
nnodes=1

# Sequence lengths - 4K response length
max_prompt_length=$((1024 * 2))  # 2K
max_response_length=$((1024 * 4))   # 4K
v_max_response_length=$((1024 * 4))  # 4K


# Batch sizes (adjusted for Intuitor and 3B model)
# 3B模型更大，适当减小batch size
train_prompt_bsz=32  # 减小batch size适应3B模型
gen_prompt_bsz=$((train_prompt_bsz * 1))
train_prompt_mini_bsz=16  # 相应减小mini batch size

# Model and data paths - 修改为Qwen2.5-3B
...
# Performance settings - 3B模型调整
gen_tp=4  # 3B模型增加tensor parallelism
micro_batch_size_per_gpu=1  # 保持为1
...
offload=True  # 3B模型可能需要offload

echo "🚀 INTUITOR CONFIGURATION:"
echo "🤖 Model: ${MODEL_PATH}"
echo "📏 Max prompt length: ${max_prompt_length}"
echo "📏 Max response length: ${max_response_length}"
echo "📏 Validation max response length: ${v_max_response_length}"
...
PYTHONUNBUFFERED=1 /data/xuandong_zhao/anaconda3/envs/archer/bin/python -m verl.trainer.main_ppo \
