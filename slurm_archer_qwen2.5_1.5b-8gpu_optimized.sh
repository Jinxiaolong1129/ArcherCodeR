...
#SBATCH --output=slurm_archer_qwen2.5_1.5b_optimized_out_%j.txt
#SBATCH --error=slurm_archer_qwen2.5_1.5b_optimized_error_%j.txt
#SBATCH --job-name=optimized-archer-qwen2.5-1.5b
...
# Activate conda environment (ensure Python 3.10)
...
# Download and install flash-attention wheel file (this will satisfy the requirements.txt dependency)
...
# Install project with dependencies (flash-attention will be skipped since already installed)
pip install -e .
...
# export VLLM_USE_V1=1  # 注释掉，与外部执行器不兼容
...
export NCCL_IB_TIMEOUT=200  # 从50增加到200
export NCCL_TIMEOUT=300000  # 大幅增加总超时
...
# Usage: sbatch slurm_archer_qwen2.5_1.5b-8gpu_optimized.sh

# sbatch --nodelist=compute-891  slurm_archer_qwen2.5_1.5b-8gpu_optimized.sh
